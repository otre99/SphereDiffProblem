...
def get_non_divergent_wind(u, v, R, tol=1e-7, atol=1e-5, maxiter=1000, display=False, method="lgmres"):
    ...
    if method == "bicgstab":
        PHI = spla.bicgstab(sparse_matrix, Q.get_flatten(), maxiter=maxiter, tol=tol, atol=atol, callback=callback, M=M)    
    if method == "gcrotmk":
        PHI = spla.gcrotmk(sparse_matrix, Q.get_flatten(), maxiter=maxiter, tol=tol, atol=atol, callback=callback, M=M)    
    if method == "minres":
        PHI = spla.minres(sparse_matrix, Q.get_flatten(), maxiter=maxiter, tol=tol, callback=callback)    
    if method == "cgs":
        PHI = spla.cgs(sparse_matrix, Q.get_flatten(), maxiter=maxiter, tol=tol, callback=callback, M=M)    
    else:
